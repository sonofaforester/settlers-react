...
const mapDispatchToProps = (dispatch: any, ownProps: any) => ({
    ...
    dispatchDistributeResources: (dieRoll: number) =>
        dispatch(distributeResources(dieRoll)),
    dispatchEndPlayerTurn: () => dispatch(endPlayerTurn()),
    dispatchInitialMove1: (townVertex: number, roadEdge: IEdge) =>
        dispatch(initialMove1(townVertex, roadEdge)),
    dispatchInitialMove2: (townVertex: number, roadEdge: IEdge) =>
        dispatch(initialMove2(townVertex, roadEdge)),
    dispatchPlayerNames: (playerNames: any) =>
...
class App extends React.Component<IAppProps, IAppState> {
    ...
    public tick() {
        ...
        if (turn >= 0 && turn < numPlayers) {
            // INITIAL MOVES 1
            const { townVertex, roadEdge } = currentPlayer.makeInitialMove1(
            ...
            // INITIAL MOVES 2
            const { townVertex, roadEdge } = currentPlayer.makeInitialMove2(
                this.props.gameState
            )
