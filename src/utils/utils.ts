import {
    ...
    INITIALIZE_GAME, MOVE_THIEF, OFFER_TRADE, PLAY_KNIGHT_CARD, UPGRADE_TOWN
} from '../actions';
import {
    Color, ICatanState, IEvent, IPlayerResources, ITradingResources, Resource, Terrain
} from '../types';
...
export function getCurrentPlayerColor(state: ICatanState) {
    ...
    if (
        state.turn >= state.playerColors.length &&
        state.turn < state.playerColors.length * 2
    ) {
        const reversePlayerNum =
            state.playerColors.length -
            (state.turn % state.playerColors.length) -
            1
...
// HIDE CERTAIN ASPECTS OF OPPONENTS STATE
/*
